...
def group_table(edges):
    ...
    for i in range(len(edges) - 1):
        ...
        if n_g == 0:
            continue
        # 用组内 95% 分位 BMI 算 95% 达标时间（覆盖组内95%孕妇，避免被极端BMI拖高）
        b_rep = np.percentile(bmi_arr[in_g], 95)
        t95_g = T_q(b_rep, 0.95)
        t50_g = T_q(bmi_arr[in_g].mean(), 0.5)
        rows.append((lo, hi, n_g, bmi_arr[in_g].mean(), t50_g, t95_g))
    return rows
...
for i, (lo, hi, n_g, bmean, t50g, t95g) in enumerate(g_rows):
    mu = alpha + beta * bmean
    cdf = norm.cdf((np.log(tt) - mu) / sigma)
...
ax.set_xlabel('孕周（周）'); ax.set_ylabel('累计达标比例')
ax.set_title('各 BMI 组的达标时间累计分布（模型估计）')
ax.legend(fontsize=9)
style_ax(ax); savefig(fig, 'fig4_达标时间CDF.png')

# --------------------------- 7. 蒙特卡洛误差分析 ---------------------------
# 估计 Y 浓度测量噪声 σ_Y：用组内残差近似（此处取混合模型残差尺度约 0.017）
sigma_Y = 0.017
Nmc = 200
...
def mc_best_times(sigma_y, seed=1):
    ...
    for _ in range(Nmc):
        # 加噪声 → 重判删失
        tmp = male.copy()
        tmp['Y'] = tmp['Y'] + rng2.normal(0, sigma_y, len(tmp))
        _recs = []
        for w, g in tmp.groupby('woman'):
            g = g.sort_values('GA')
            ys = g['Y'].values; gas = g['GA'].values
            bmi = g['BMI'].iloc[0]
            if ys[0] >= THRESH:
                _recs.append((bmi, 'L', -np.inf, gas[0]))
            elif ys[-1] < THRESH:
                _recs.append((bmi, 'R', gas[-1], np.inf))
            else:
                i = int(np.where((ys[:-1] < THRESH) & (ys[1:] >= THRESH))[0][0])
                _recs.append((bmi, 'I', gas[i], gas[i + 1]))
        try:
        ...
        # 用相同分组边界算各组 95% 达标时间
        row_t = []
        for lo, hi, _, _, _, _ in g_rows:
            in_g = (bmi_arr >= lo) & (bmi_arr < hi)
            if in_g.sum() == 0:
                row_t.append(np.nan); continue
            b_rep = np.percentile(bmi_arr[in_g], 95)
            row_t.append(np.exp(a + b * b_rep + norm.ppf(0.95) * s))
        times.append(row_t)
...
fig, ax = plt.subplots(figsize=(7, 4.5))
sigma_levels = [0.005, 0.01, 0.017, 0.025, 0.035]
...
def fmt_group_table(rows, include_count=True):
    lines = ['| BMI区间 | 人数 | 组内平均BMI | 中位达标周 | 95%达标周(最佳时点) |',
             '|---|---|---|---|---|']
    for lo, hi, n_g, bmean, t50g, t95g in rows:
        lines.append('| [%.1f, %.1f) | %d | %.1f | %.1f | **%.1f** |'
                     % (lo, hi, n_g, bmean, t50g, t95g))
    return '\n'.join(lines)
...
## 2. 分组结果
采用等频分位分组（%d 组），每组 BMI 区间与最佳时点见指标汇总表。
